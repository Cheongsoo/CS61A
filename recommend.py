...
def find_closest(location, centroids):
    ...
    """
    # BEGIN Question 3
    return min(centroids, key=lambda centroids: distance(location, centroids))
    # END Question 3
...
def k_means(restaurants, k, max_updates=100):
    ...
    while old_centroids != centroids and n < max_updates:
        ...
        centroids = [find_centroid(element) for element in clusters]
        # END Question 6
        n += 1
    return centroids
...
def best_predictor(user, restaurants, feature_fns):
    ...
    feature_pred = [find_predictor(user, reviewed, feature_fn) for feature_fn in feature_fns]  

    return max(feature_pred, key = lambda f: f[1])[0]
