...
def two_list(lst1, lst2):
    ...
    def helper(count, index):
        if count == 0:
            if index + 1 == len(lst1):
                return Link.empty
            return Link(lst1[index + 1], helper(lst2[index + 1] - 1, index + 1))
        return Link(lst1[index], helper(count - 1, index))
    return helper(lst2[0], 0)
...
def cumulative_sum(t):
    ...
    """
    old = [cumulative_sum(b) for b in t.branches]
    new = sum(b.root for b in b.branches) + t.root
    return Tree(new, old)
